...
- `data/people.json` — 재벌 표본 인물 목록. 포브스 코리아 2026 50 Richest 중 CC 라이선스
  사진을 확보한 실명 인물(현재 14명, 순위·자산·업적·사진 출처 포함)과, 인구 통계(평균/표준편차)
  표본을 두텁게 하기 위한 익명 인물(현재 50명, 이름 미표시)로 구성됩니다.
- 실명 인물 사진은 전부 위키미디어 커먼즈 등 **재사용 가능한 라이선스**(CC BY/CC BY-SA/CC0 등)만
  사용합니다 — 애드센스로 수익화되는 사이트라 저작권 있는 언론/파파라치 사진은 쓰지 않습니다.
  포브스 상위 인물 상당수는 CC 라이선스 사진 자체가 없어 전원을 실명화하지는 못했습니다.
...
`assets/richKR/` 안의 실명 인물 사진은 각각 별도의 CC 라이선스를 따르며, 출처는
`data/people.json`의 `credit` 필드와 결과 화면에 표기되어 있습니다.
